+++
title = "SecCookieFormat"
weight = 20
+++

This chapter details the configuration directives in SecLang++.
...
> **Description:** 配置是否允许 WAF 访问请求体内容。
...
> **Description:** 配置不包含文件上传的请求体最大大小 (字节)。
...
> **Description:** 配置当请求体超过 SecRequestBodyLimit 限制时的处理方式。
...
> **Description:** 配置是否允许 WAF 访问响应体内容。
...
> **Description:** 配置需要检查的响应体 MIME 类型。
...
> **Description:** 指定应用于 application/x-www-form-urlencoded 内容的参数分隔符字符。
...
> **Description:** 配置 PCRE 正则表达式匹配的最大次数,防止 ReDoS 攻击。
...
> **Description:** 配置 PCRE 正则表达式递归深度限制。
...
> **Description:** 配置 Unicode 映射文件路径和代码页。
...
> **Description:** 配置状态引擎，用于收集和报告运行时统计信息。
...
> **Description:** 配置请求体在内存中缓冲的最大大小，超过此限制将写入临时文件。
...
> **Description:** 配置当响应体超过 SecResponseBodyLimit 限制时的处理方式。
...
> **Description:** 配置地理位置数据库文件路径，用于 @geoLookup 操作符。
...
> **Description:** 配置 HTTP 黑名单 (Project Honeypot) API 密钥。

            
**Syntax:** `SecHttpBlKey API_KEY`

            
若 @rbl Operators use dnsbl.httpbl.org RBL（http://www.projecthoneypot.org/httpbl_api.php），则必须提供 API 密钥。该密钥绑定至个人用户账户，并将包含在 RBL DNS 请求中。
...
> **Description:** 从远程 HTTPS 服务器加载规则配置。
...
> **Description:** 配置 Web 应用程序标识符,用于区分不同的应用程序。
...
> **Description:** 配置单次请求允许上传的最大文件数量。

            
**Syntax:** `SecUploadFileLimit LIMIT`
...
> **Description:** 配置是否在请求处理完成后保留上传的文件。
